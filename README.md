...
******* readme currently not up to date, should be by tomorrow (oct 26)
...
while still giving you tools to write good and maintainable code.
...
Theoretically, the sandbox is the only thing that have access to all modules. When you work in a module, you can't access
anything from the application without passing by the api, and the api wont let you call other modules, you can however call anything within any files your module is controlling.
There is no a lot of functionality in the api currently, but this is where there is the most work to do.



## What Library stackJS uses?
None, you can use any library (even your own) with it. 
In fact it also play well with php MVC frameworks, you are absolutely not obligated to use the view provided with the example.
...
Look at the file structure of the example. Open js then modules, also open stackJS then conf. 
...
## Files names
I use the class names Model, View, Controller but this is absolutely not obligatory. You could name them Mediators, ajax or anything else.
The class name is setter in the file itself and also in conf files.
...
The framework should help you separate your javascript logic. You can refer to the recipe CRUD example provided with the framework.
...
api.loadModules(["moduleName"], function(){ callBack() })
